...
| Remote | EQ / DSP | Settings | Light theme |
|:------:|:--------:|:--------:|:-----------:|
| <img src="docs/screenshots/remote.png" width="200" alt="Remote tab"> | <img src="docs/screenshots/eq.png" width="200" alt="EQ / DSP tab"> | <img src="docs/screenshots/settings.png" width="200" alt="Settings tab"> | <img src="docs/screenshots/remote-light.png" width="200" alt="Remote tab, light theme"> |
...
- **Windows** - the `.exe` installer (recommended) or the `.msi`.
- **macOS** - the universal `.dmg` (a single file that runs on both Apple Silicon and Intel Macs).
...
Installers are **not cross-compiled** - build Windows on Windows and macOS on a Mac. Because this is a Cargo workspace, the output lands under `app/target/release/bundle/` (not `app/src-tauri/target/`).

- **Windows:** run `npm run tauri build`. The `.exe` and `.msi` installers appear under `app\target\release\bundle\` (in the `nsis\` and `msi\` folders).
- **macOS (universal):** build a single binary that runs on both Apple Silicon and Intel:
  ...
  The `.dmg` appears in `app/target/release/bundle/dmg/`.
...
Reads are open; writing DSP and host settings requires a signed `HMAC_SHA256_AES256` request, which the app builds natively in Rust.
